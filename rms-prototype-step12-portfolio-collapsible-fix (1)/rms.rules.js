/* =========================================================
    ...
    RMS.registerModule("pricingRules", {
      get storageKey(){ return RMS.util.storageKey("rms_pricing_rules_v1"); },
      get logKey(){ return RMS.util.storageKey("rms_pricing_rules_log_v1"); },
...
leadTimeTiers: [
  ...
  { minDays: 91, maxDays: 365, adjPct: 8 }
      ...
      _rebuildSelectors(RMS){
        const rtSel =         // Ensure selectors match current property DB
        this._rebuildSelectors(RMS);
        // Reload rules per property + refresh UI
        RMS.events?.on("propertyChanged", ()=>{
          try { this._rebuildSelectors(RMS); } catch(e){}
          // Re-render rules page if open
          this.render?.(RMS);
        });

document.getElementById("roomTypeSelect");
        const rpSel = document.getElementById("ratePlanSelect");
        const roomTypes = (RMS.getRoomTypes ? RMS.getRoomTypes() : (RMS.db?.data?.roomTypes || []));
        const ratePlans = (RMS.getRatePlans ? RMS.getRatePlans() : (RMS.db?.data?.ratePlans || []));
        ...
        if (rtSel){
          ...
          const desired = RMS.state.selectedRoomType;
          rtSel.value = (desired && roomTypes.includes(desired)) ? desired : (roomTypes[0] || "");
          RMS.state.selectedRoomType = rtSel.value;
        }
        ...
        if (rpSel){
          ...
          const desired = RMS.state.selectedRatePlan;
          rpSel.value = (desired && ratePlans.includes(desired)) ? desired : (ratePlans[0] || "");
          RMS.state.selectedRatePlan = rpSel.value;
        }
      },
...
,
  forecastOccupancyTiers: Array.isArray(r?.forecastOccupancyTiers) ? r.forecastOccupancyTiers.map(t => ({
    min: Number(t.min),
    max: Number(t.max),
    adjPct: Number(t.adjPct)
  })) : d.forecastOccupancyTiers
};

        if (!Number.isFinite(out.rounding) || out.rounding <= 0) out.rounding = d.rounding;

// Pricing mode
const allowedModes = new Set(["manual", "occ", "occ_lead", "forecast_occ"]);
...
if (!out.leadTimeTiers.length) out.leadTimeTiers = d.leadTimeTiers;
        ...
        // Clean forecast tiers
        out.forecastOccupancyTiers = (out.forecastOccupancyTiers || [])
          .filter(t => Number.isFinite(t.min) && Number.isFinite(t.max) && Number.isFinite(t.adjPct))
          .map(t => ({
            min: Math.max(0, Math.min(100, Math.round(t.min))),
            max: Math.max(0, Math.min(100, Math.round(t.max))),
        ...
        if (!out.forecastOccupancyTiers.length) out.forecastOccupancyTiers = d.forecastOccupancyTiers;
      ...
      getModeLabel(mode){
        ...
        if (m === "forecast_occ") return "Forecast occupancy";
        return m || "—";
      },

      // Public API consumed by the calendar view
      recommendPrice(current, occPercent, dateKey = null, roomType = null){
  ...
  const getTierAdj = (tiers, occ) => {
    const o = Number(occ);
    const tier = (tiers || []).find(t => o >= t.min && o <= t.max) || null;
    return { tier, adjPct: tier ? Number(tier.adjPct) : 0, occ: o };
  };

  // Choose occupancy basis
  let occBasis = "current";
  let occUsed = Number(occPercent);
  let tiers = rules.occupancyTiers;

  if (mode === "forecast_occ" && dateKey) {
    occBasis = "forecast";
    tiers = rules.forecastOccupancyTiers || rules.occupancyTiers;
    const fc = RMS?.modules?.forecastPace;
    const fcOcc = (fc && typeof fc.getForecastOccPercent === 'function')
      ? fc.getForecastOccPercent(RMS, dateKey, roomType || "__all__")
      : null;
    if (Number.isFinite(fcOcc)) occUsed = Number(fcOcc);
  }

  const occRes = getTierAdj(tiers, occUsed);
  const occAdjPct = occRes.adjPct;
  ...
  return Math.round(rec / round) * round;
      ...
      explainRecommendation(current, occPercent, dateKey = null, roomType = null){
        if (current == null) return null;
        const rules = this.getRules();
        const mode = rules.pricingMode || "occ";
        ...
        if (mode === "forecast_occ" && dateKey) {
          ...
          const fcOcc = (fc && typeof fc.getForecastOccPercent === 'function')
            ...
            : null;
          if (Number.isFinite(fcOcc)) occUsed = Number(fcOcc);
          occTier = (rules.forecastOccupancyTiers || []).find(t => occUsed >= t.min && occUsed <= t.max) || null;
          occAdjPct = occTier ? Number(occTier.adjPct) : 0;
        } else {
          occTier = (rules.occupancyTiers || []).find(t => occUsed >= t.min && occUsed <= t.max) || null;
          occAdjPct = occTier ? Number(occTier.adjPct) : 0;
        }
        ...
        return {
          ...
          recommended,
          rounding: round,
          occPercent: Number(occPercent),
          occUsedPercent: occUsed,
          occTier,
          occAdjPct,
          leadDays,
          leadTier,
      ...
      init(RMS){
        ...
        RMS.pricing = {
          ...
          recommendPrice: (current, occPercent, dateKey = null, roomType = null) => this.recommendPrice(current, occPercent, dateKey, roomType),
          explainRecommendation: (current, occPercent, dateKey = null, roomType = null) => this.explainRecommendation(current, occPercent, dateKey, roomType),
...
document.getElementById("btnRulesAddForecastTier")?.addEventListener("click", () => {
  const cur = this.readFromUI({ validate: false }) || this.getRules();
  cur.forecastOccupancyTiers = cur.forecastOccupancyTiers || [];
  cur.forecastOccupancyTiers.push({ min: 0, max: 0, adjPct: 0 });
  this.render(RMS, cur);
});

document.getElementById("btnRulesResetForecast")?.addEventListener("click", () => {
  const cur = this.readFromUI({ validate: false }) || this.getRules();
  const defaults = this.defaultRules();
  cur.forecastOccupancyTiers = defaults.forecastOccupancyTiers;
  this.render(RMS, cur);
});
...
document.getElementById("rulesForecastTierBody")?.addEventListener("click", (e) => {
  const btn = e.target?.closest?.('button[data-action="delete-forecast-tier"]');
  if (!btn) return;
  const idx = Number(btn.dataset.idx);
  const cur = this.readFromUI({ validate: false });
  if (!cur) return;
  cur.forecastOccupancyTiers = cur.forecastOccupancyTiers || [];
  if (Number.isFinite(idx)) cur.forecastOccupancyTiers.splice(idx, 1);
  this.render(RMS, cur);
});
...
, (e) => {
      ...
      readFromUI({ validate = true } = {}){
        ...
        // Forecast occupancy tiers
        const forecastTbody = document.getElementById("rulesForecastTierBody");
        const forecastRows = forecastTbody ? [...forecastTbody.querySelectorAll("tr")] : [];
        const forecastOccupancyTiers = forecastRows.length ? forecastRows.map((tr) => {
          const min = Number(tr.querySelector('input[data-field="min"]')?.value);
          const max = Number(tr.querySelector('input[data-field="max"]')?.value);
          const adjPct = Number(tr.querySelector('input[data-field="adjPct"]')?.value);
          return { min, max, adjPct };
        }) : (this.getRules().forecastOccupancyTiers || this.defaultRules().forecastOccupancyTiers);

        if (validate) {
          for (const t of forecastOccupancyTiers) {
            if (!Number.isFinite(t.min) || !Number.isFinite(t.max) || !Number.isFinite(t.adjPct)) {
              window.RMS?.ui?.toast?.("Please fill in all forecast tier values.", { title: "Validation" });
              return null;
            }
            if (t.min > t.max) {
              window.RMS?.ui?.toast?.("Forecast tier min occupancy must be ≤ max occupancy.", { title: "Validation" });
              return null;
            }
          }
        } else {
...
const rounding = Number(document.getElementById("ruleRounding")?.value);
        ...
        return {
          ...
          forecastOccupancyTiers: forecastOccupancyTiers
        };
      },
  ...
  if (modeNote) {
    ...
    } else if (editingMode === "occ_lead") {
      detail = "Recommendations increase with both lead time (farther dates) and occupancy.";
    } else if (editingMode === "forecast_occ") {
      detail = "Recommendations adjust price based on forecasted occupancy tiers (not current occupancy).";
    } else {
      detail = "Recommendations use the selected pricing mode to generate target prices.";
    }
  ...
  const forecastCard = document.getElementById("rulesForecastCard");
  const showForecast = (editingMode === "forecast_occ");
  if (forecastCard) forecastCard.classList.toggle("hidden", !showForecast);
  ...
  // Forecast occupancy tiers table
  const fcBody = document.getElementById("rulesForecastTierBody");
  if (fcBody) {
    fcBody.innerHTML = (rules.forecastOccupancyTiers || []).map((t, i) => `
      <tr>
        ...
        <td style="display:flex; gap:10px; align-items:center;">
          ...
          <button class="button" data-action="delete-forecast-tier" data-idx="${i}" style="padding:6px 10px;">Delete</button>
        </td>
      </tr>
    `).join("");
  }
...
RMS.registerModule("coreCalendar", {
      ...
      render(RMS){
        // Multi-property safety: ensure current selections are valid after a property switch.
        // If selections are null or not in the enabled scope, default to the first available option.
        const rooms = (RMS.getRoomTypes?.() || RMS.db?.data?.roomTypes || []);
        const rates = (RMS.getRatePlans?.() || RMS.db?.data?.ratePlans || []);
        if (!RMS.state.selectedRoomType || (rooms.length && !rooms.includes(RMS.state.selectedRoomType))) {
          RMS.state.selectedRoomType = rooms[0] || null;
        }
        if (!RMS.state.selectedRatePlan || (rates.length && !rates.includes(RMS.state.selectedRatePlan))) {
          RMS.state.selectedRatePlan = rates[0] || null;
        }
  ...
  applyBtn.title = (pm === "manual") ? "Manual pricing mode: no recommendation to apply." : "";
      ...
      renderCalendarGrid(RMS){
        ...
        const cellHTML = (dayNumber, isMuted=false) => {
          ...
          // Forecast occupancy fill (lighter). Uses Forecast module if available.
          let fc = 0;
          try{
            const fmod = RMS.modules && RMS.modules.forecastPace;
            if(fmod && typeof fmod.getForecastOccPercent === "function"){
              const dkTmp = this.getDateKey(dayNumber);
              const fcOcc = fmod.getForecastOccPercent(RMS, dkTmp, roomType);
              if(fcOcc !== null && fcOcc !== undefined && isFinite(fcOcc)){
                fc = Math.max(0, Math.min(100, Number(fcOcc)));
              }
            }
          }catch(e){ fc = 0; }
          ...
          return `
            <div class="day" role="button" tabindex="0" data-day="${dayNumber}">
              <div class="fcfill" style="height:${fc}%;"></div>
...
${otherHead}
        ...
        const recommend = (current, occPercent, dateKey, roomType) => RMS.pricing?.recommendPrice?.(current, occPercent, dateKey, roomType);
        ...
        RMS.db.data.roomTypes.forEach((room) => {
          ...
          const rec = sold ? null : recommend(current, occP, dk, room);
