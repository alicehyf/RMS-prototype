...
(function(){
  ...
  const runCycle = (st, reason = "auto") => {
    ...
    st.lastRunSummary = { simulated: true, reason };
    save(st);
    setStatus(st);
    appendRunLog(st, reason === "manual" ? "Manual run executed (simulated)." : "Simulated run completed (checked data)." );
    if (reason === "manual") {
        ...
        enabled.addEventListener("change", async () => {
          ...
          if (wantEnable) {
            ...
            const msg =
              ...
              `While enabled, the system will simulate a scheduled run every ${mins} minutes (until you disable it).\n\n` +
              `This is a surface-level demo: it will not run a real backend job.`;
