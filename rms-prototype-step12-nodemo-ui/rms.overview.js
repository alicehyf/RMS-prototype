/* =========================================================
    ...
    RMS.registerModule("kpiDashboard", {
      init(RMS){
        ...
        const pricingRestrictionsView = document.getElementById("pricingRestrictionsView");
        const autopilotView = document.getElementById("autopilotView");
...
const show = (which) => {
          ...
          const isRestr = (which === "restrictions");
          const isAuto = (which === "autopilot");
          const isProps = (which === "properties");
if (dashboardView) dashboardView.classList.toggle("hidden", !isDash);
          if (forecastView) forecastView.classList.toggle("hidden", !isForecast);
          ...
          if (pricingRestrictionsView) pricingRestrictionsView.classList.toggle("hidden", !isRestr);
          if (autopilotView) autopilotView.classList.toggle("hidden", !isAuto);
          if (propertiesView) propertiesView.classList.toggle("hidden", !isProps);
          if (isRules) {
          ...
          if (isCal) {
            ...
            const prx = RMS.modules.pricingRestrictions;
            prx?.render?.(RMS);
          } else if (isAuto) {
            const ap = RMS.modules.autopilot;
            ap?.render?.(RMS);
          } else if (isForecast) {
...
rmsToggle?.addEventListener('click', () => {
  ...
  setRmsOpen(open);
        ...
        const setActiveMenu = (which) => {
          ...
          const isRms = (["overview","forecast","autopilot","calendar","rules","restrictions"].includes(which));
          const isSettings = (["properties"].includes(which));
          if (isRms) {
            setRmsOpen(true);
...
document.getElementById("btnGoCalendar")?.addEventListener("click", () => {
        ...
        document.getElementById("btnForecastGoCalendar")?.addEventListener("click", () => {
          go("calendar");
          RMS.ui?.toast?.("Calendar view", { title: "Navigation" });
        });
        ...
        document.getElementById("btnGoProperties")?.addEventListener("click", () => {
          go("properties");
          RMS.ui?.toast?.("Properties", { title: "Settings" });
        });

        
        
        // =========================
        // Portfolio snapshot: global show/hide (all pages)
        // =========================
        const PORTFOLIO_VIS_KEY = "rms_portfolio_visible_v1";
        const portfolioCard = document.getElementById("portfolioCard");
        const btnTogglePortfolio = document.getElementById("btnTogglePortfolio");
        const btnHidePortfolio = document.getElementById("btnHidePortfolio");

        function isPortfolioVisible(){
          const v = localStorage.getItem(PORTFOLIO_VIS_KEY);
          if (v === null) return true; // default visible
          return v === "1";
        }
        function setPortfolioVisible(on){
          localStorage.setItem(PORTFOLIO_VIS_KEY, on ? "1" : "0");
          syncPortfolioUI();
        }
        function syncPortfolioUI(){
          const on = isPortfolioVisible();
          if (portfolioCard) portfolioCard.classList.toggle("hidden", !on);
          if (btnTogglePortfolio) btnTogglePortfolio.style.opacity = on ? "1" : ".65";
          if (btnTogglePortfolio) btnTogglePortfolio.title = on ? "Hide portfolio snapshot" : "Show portfolio snapshot";
        }

        btnTogglePortfolio?.addEventListener("click", () => setPortfolioVisible(!isPortfolioVisible()));
        btnHidePortfolio?.addEventListener("click", () => setPortfolioVisible(false));
...
// KPI range toggle (Overview only)
      ...
      renderPortfolioSnapshot(RMS){
        const tb = document.querySelector("#portfolioTable tbody");
        if (!tb) return;
        const props = RMS.state.properties || [];
        const dbMap = RMS.db?._dbByProperty || {};
        const curSym = (pid)=>{
          const p = props.find(x=>x.id===pid);
          const map = { IDR:"Rp", USD:"$", EUR:"€", GBP:"£", SGD:"S$", AUD:"A$", CNY:"¥", JPY:"¥" };
          const c = (p?.currency||"IDR").toUpperCase();
          return map[c] || (c + " ");
        };
        const fmt = (pid, n)=>{
          const sym = curSym(pid);
          try { return sym + Number(n||0).toLocaleString(); } catch(e){ return sym + (n||0); }
        };
        const compute = (dbObj)=>{
          const inv = dbObj?.inventory || {};
          const prices = dbObj?.prices || {};
          const roomTypes = dbObj?.roomTypes || [];
          const plans = dbObj?.ratePlans || [];
          const plan = plans[0];
          const dates = Object.keys(inv).filter(k=>/^\d{4}-\d{2}-\d{2}$/.test(k)).sort().slice(0,30);
          let total=0, sold=0, revenue=0;
          dates.forEach(dk=>{
            const invDay = inv[dk] || {};
            roomTypes.forEach(rt=>{
              const cell = invDay[rt];
              if (!cell) return;
              const t = cell.total||0;
              const rem = cell.remaining||0;
              const s = Math.max(0, t-rem);
              total += t; sold += s;
              const p = prices?.[dk]?.[rt]?.[plan];
              if (p) revenue += s * p;
            });
          });
          const occ = total? (sold/total):0;
          const adr = sold? (revenue/sold):0;
          const revpar = total? (revenue/total):0;
          return { occ, adr, revpar, revenue };
        };
        tb.innerHTML = "";
        props.forEach(p=>{
          const dbObj = dbMap[p.id];
          if (!dbObj) return;
          const k = compute(dbObj);
          const tr = document.createElement("tr");
          tr.innerHTML = `
            <td style="text-align:left;">${p.name}</td>
            <td style="text-align:right;">${Math.round((k.occ||0)*100)}%</td>
            <td style="text-align:right;">${fmt(p.id, Math.round(k.adr||0))}</td>
            <td style="text-align:right;">${fmt(p.id, Math.round(k.revpar||0))}</td>
            <td style="text-align:right;">${fmt(p.id, Math.round(k.revenue||0))}</td>
            <td style="text-align:left;"><button class="button" data-switch-prop="${p.id}" style="padding:6px 10px;">Switch</button></td>
          `;
          tb.appendChild(tr);
        });
        tb.querySelectorAll("[data-switch-prop]")?.forEach(btn=>{
          btn.addEventListener("click", ()=>{
            const pid = btn.getAttribute("data-switch-prop");
            RMS.setActiveProperty(pid);
          });
        });
      },
      ...
      render(RMS){
        ...
        try { this.renderPortfolioSnapshot(RMS); } catch(e){}
        if (!k) return;
