...
if (iDay[room].remaining <= 0) {
  ...
  cDay[room][rp] = { old, now: autoPrice, when: whenLabel };
        ...
        /* -------------------------------
           Multi-property DB containers
        -------------------------------- */
        const dbKomodo = db;

        // Create a second property with slightly different baseline (demo only)
        const dbBali = JSON.parse(JSON.stringify(dbKomodo));

        // Adjust Bali pricing & inventory so switching feels real
        const _round = (n, step=5000) => (n==null? null : Math.round(n/step)*step);
        Object.keys(dbBali.prices).forEach(dk=>{
          const byRoom = dbBali.prices[dk] || {};
          Object.keys(byRoom).forEach(rt=>{
            const byRate = byRoom[rt] || {};
            Object.keys(byRate).forEach(rp=>{
              const v = byRate[rp];
              if (typeof v === "number") byRate[rp] = _round(v * 1.12, 5000);
            });
          });
        });
        Object.keys(dbBali.inventory).forEach(dk=>{
          const byRoom = dbBali.inventory[dk] || {};
          Object.keys(byRoom).forEach(rt=>{
            const inv = byRoom[rt];
            if (!inv) return;
            inv.total = Math.max(1, (inv.total || 0) + 2);
            inv.remaining = Math.max(0, Math.min(inv.total, (inv.remaining || 0) + 1));
            inv.delta24h = (inv.delta24h || 0) + ((parseInt(dk.slice(-2),10) % 3) - 1);
          });
        });

        const templates = { komodo: dbKomodo, bali: dbBali };

        const dbByProperty = { ...templates };

        // Point "db" to active property (keeps existing code compatible)
        db = dbByProperty[RMS.state.activePropertyId] || dbKomodo;

// Public API
        RMS.db = {
          data: db,
          _dbByProperty: dbByProperty,
          _templates: templates,


          // --- Persistence (localStorage) ---
          // Bump version to avoid older cached snapshots that could mark everything as sold out.
          _baseKey: "rms_db_v3",
          _key(pid){ return `${this._baseKey}::${pid}`; },
          get storageKey(){ return this._key(RMS.state.activePropertyId || "komodo"); },
          autosave: true,
          _saveTimer: null,
          ...
          _applySnapshot(snapshot, targetDb = db){
            // mutate in place to keep references stable
            Object.keys(targetDb).forEach(k => { if (!(k in snapshot)) delete targetDb[k]; });
            Object.keys(snapshot).forEach(k => { targetDb[k] = snapshot[k]; });
          },

          load(pid = (RMS.state.activePropertyId || "komodo")){
            try{
              ...
              // basic snapshot sanity check (avoid corrupted/empty saved state)
              if (!snap.inventory || Object.keys(snap.inventory).length < 5) return false;
              if (!snap.baseFlexibleOcean || Object.keys(snap.baseFlexibleOcean).length < 5) return false;

              // Stronger sanity: ensure there is at least SOME availability (otherwise everything renders sold out)
              // and at least some non-null base prices.
              let totalRemaining = 0;
              let checkedInv = 0;
              Object.keys(snap.inventory).slice(0, 120).forEach(dk => {
                const byRoom = snap.inventory[dk] || {};
                Object.keys(byRoom).forEach(rt => {
                  const inv = byRoom[rt];
                  if (!inv) return;
                  checkedInv++;
                  totalRemaining += (inv.remaining || 0);
                });
              });
              const hasSomeBase = Object.values(snap.baseFlexibleOcean).some(v => typeof v === "number" && v > 0);
              if (!hasSomeBase) return false;
              if (checkedInv > 0 && totalRemaining <= 0) return false;
              ...
              // Repair missing/zero prices (can happen after migrations or partial snapshots)
              this._repairPrices(db);
          ...
          ensureProperty(pid, { cloneFrom } = {}){
            if (!pid) pid = "komodo";
            if (this._dbByProperty[pid]) return this._dbByProperty[pid];
            const base = this._clone(this._templates[cloneFrom] || this._templates[pid] || this._templates.komodo);
            this._dbByProperty[pid] = base;
            return base;
          },
          ...
          _sumRemaining(anyDb){
            try{
              const inv = anyDb?.inventory;
              if (!inv) return 0;
              let sum = 0;
              Object.keys(inv).slice(0, 180).forEach(dk=>{
                const byRoom = inv[dk] || {};
                Object.keys(byRoom).forEach(rt=>{
                  const v = byRoom[rt];
                  if (!v) return;
          ...
          _repairPrices(anyDb = db){
            try{
              const prices = anyDb.prices || (anyDb.prices = {});
              const inv = anyDb.inventory || {};
              const baseMap = anyDb.baseFlexibleOcean || {};
              // Ensure every rate plan has a non-zero price when inventory exists.
              Object.keys(inv).forEach(dk=>{
                const byRoomInv = inv[dk] || {};
                const base = baseMap[dk];
                Object.keys(byRoomInv).forEach(rt=>{
                  const invObj = byRoomInv[rt];
                  if (!invObj) return;
                  // Only repair dates/rooms that have availability
                  if ((invObj.remaining || 0) <= 0) return;

                  prices[dk] = prices[dk] || {};
                  prices[dk][rt] = prices[dk][rt] || {};

                  (anyDb.ratePlans || ratePlans).forEach(rp=>{
                    const cur = prices[dk][rt][rp];
                    if (typeof cur === "number" && cur > 0) return;

                    // derive from base flexible ocean if available, otherwise carry forward last known
                    let derived = null;
                    const rm = roomMult[rt] ?? 1.0;
                    const rtm = rateMult[rp] ?? 1.0;

                    if (typeof base === "number" && base > 0){
                      derived = roundTo(base * rm * rtm, 5000);
                    } else {
                      // carry forward from previous dates if base is missing (should be rare)
                      const keys = Object.keys(prices).filter(k=>k < dk).sort();
                      for (let i=keys.length-1; i>=0; i--){
                        const prev = prices[keys[i]]?.[rt]?.[rp];
                    ...
                    if (derived != null){
                      prices[dk][rt][rp] = derived;
                    } else {
                      // last resort: do not force a number if we truly can't derive
                      prices[dk][rt][rp] = null;
                    }
                  });
                });
              });
          ...
          setActive(pid, { load = true, cloneFrom } = {}){
            const target = this._dbByProperty[pid]
              || (this._dbByProperty[pid] = this._clone(this._templates[pid] || this._templates[cloneFrom] || this._templates.komodo));
            db = target;
            this.data = db;

            if (load) this.load(pid);

            // Ensure prices are never shown as 0 when availability exists
            this._repairPrices(db);


            // If a cached snapshot makes everything sold out (e.g., bad storage), fall back to the seeded template.
            if (this._sumRemaining(db) <= 0){
              const fallback = this._clone(this._templates[pid] || this._templates[cloneFrom] || this._templates.komodo);
              this._applySnapshot(fallback, db);
              this._repairPrices(db);
        ...
        // Initialize active property DB (per-property localStorage)
        RMS.db.setActive(RMS.state.activePropertyId || "komodo", { load: true });

        // Respond to property switches
        RMS.events?.on("propertyChanged", ({ propertyId })=>{
          RMS.db.setActive(propertyId, { load: true });
        });
