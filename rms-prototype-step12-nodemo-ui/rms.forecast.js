...
RMS.registerModule("forecastPace", {
  init(RMS){
        RMS.events?.on("propertyChanged", ()=>{
          try { this._loadUI?.(); } catch(e){}
          this.render?.(RMS);
        });

    this._uiKeyBase = "rms_forecast_ui_v1";
    this._asOf = null;
    this._room = "__all__";
    this._window = 30;
    ...
    this._loadUI = () => {
      try{
        const raw = localStorage.getItem(RMS.util.storageKey(this._uiKeyBase));
        if (raw){
          const st = JSON.parse(raw);
          if (st && typeof st === "object"){
            if (st.asOf !== undefined) this._asOf = st.asOf;
            if (st.room !== undefined) this._room = st.room;
            if (st.window !== undefined) this._window = st.window;
          }
        }
      }catch(e){}
    };
    ...
    this._saveUI = () => {
      try{
        localStorage.setItem(
          RMS.util.storageKey(this._uiKeyBase),
          JSON.stringify({ asOf: this._asOf, room: this._room, window: this._window })
        );
      }catch(e){}
    };

    // Restore UI state (per-property)
    this._loadUI();
...
// Restore UI state
    ...
    const saveUI = () => {
      try{
        localStorage.setItem(RMS.util.storageKey(this._uiKeyBase), JSON.stringify({ asOf: this._asOf, room: this._room, window: this._window }));
      }catch(e){ /* ignore */ }
    };
